...
public:  
  ...
  bool sanity() const{  
    int size = 0;  
    int height;  
    T max_tree = MAX_node(root);  
    T min_tree = MIN_node(root);  
    return (helper(root, size, height, max_tree, min_tree)>0 && size == the_size);  
    }  
...
bool helper(node* node, int& size, int& height, int maxim, int minim) const{  
...
if(node->data < minim || node->data > maxim)  
   return false;  
...
bool kids_check = helper(node->left, size, L_Height, node->data, minim) && helper(node->right, size, R_Height, maxim, node->data);  
  
height = 1 + max(L_Height, R_Height);  
  
int space = R_Height - L_Height;  
  
if (space > 1 || space < -1)  
   return false;  
  
if (((space == -1) && (node->balance != balance_type(LH))) || ((space == 0) && (node->balance != balance_type(EH))) || ((space == 1) && (node->balance != balance_type(RH))))  
   return false ;  
